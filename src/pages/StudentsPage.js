...
export default function StudentsPage() {
  ...
  const filtered = students.filter(s => {
    ...
    const matchFilter =
      filter === 'All' ? true :
      filter === 'Paid' ? s.paid :
      filter === 'Pending' ? !s.paid :
      filter === 'Expiring soon' ? getDaysLeft(s.endDate) <= 7 :
      filter === 'No biometric' ? !s.bioRegistered : true;
  ...
  return (
    <div style={{ padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 16, height: 'calc(100vh - 64px)', overflow: 'hidden' }}>
      {/* Toolbar */}
      ...
      <div style={{ display: 'flex', gap: 16, flex: 1, overflow: 'hidden' }}>
        {/* Grid */}
        <div style={{ width: 360, flexShrink: 0, overflowY: 'auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, alignContent: 'start' }}>
          {filtered.length === 0 ? (

...
export const PlanPill = ({ plan }) => {
  ...
  return (
    <span style={{ background: s.bg, color: s.text, fontSize: 11, fontWeight: 500, padding: '3px 10px', borderRadius: 6 }}>
      {s.label}
    </span>
  );
};
...
export const PaymentBadge = ({ status }) => {
  ...
  return (
    <span style={{ background: m.bg, color: m.text, fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 6 }}>
      {m.label}
    </span>
  );
};
...
export const BioDot = ({ registered }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
    <div style={{ width: 7, height: 7, borderRadius: '50%', background: registered ? '#1D9E75' : '#E24B4A', flexShrink: 0 }} />
    <span style={{ fontSize: 11, color: '#6B6860' }}>{registered ? 'Bio reg.' : 'No bio'}</span>
  </div>
);

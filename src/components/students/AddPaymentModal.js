...
export default function AddPaymentModal({ student, onClose }) {
  ...
  const handleSubmit = () => {
    ...
    if (amt > remaining) {
      setError(`Max payable is ${formatCurrency(remaining)}`);
      return;
    }
  ...
  return (
    <Modal title={`Add payment — ${student.name}`} onClose={onClose}>
      ...
      >
        ...
        >
          <span style={{ color: "#6B6860" }}>Total due</span>
          <span style={{ fontWeight: 600, color: "#1A1917" }}>
            {formatCurrency(total)}
          </span>
        </div>
        {(student.carriedForward || 0) > 0 && (
          <div
            style={{
              fontSize: 11,
              color: "#854F0B",
          ...
          >
            Includes ₹{student.carriedForward.toLocaleString("en-IN")} carried
            from previous cycle
          </div>
        )}
        <div
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, color: "#6B6860", marginBottom: 8 }}>
          Quick fill
        </div>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {[remaining, Math.floor(remaining / 2), 500, 1000]
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <input
          ...
          style={{
            ...inputStyle,
            borderColor: error ? "#E24B4A" : "rgba(0,0,0,0.12)",
          }}
          value={amount}
          onChange={(e) => {

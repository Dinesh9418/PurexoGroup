...
export default function AddStudentModal({ onClose }) {
  ...
  const readOnlyStyle = { ...inputStyle, background: '#F3F2EF', color: '#6B6860', cursor: 'not-allowed' };
  const labelStyle = { fontSize: 12, color: '#6B6860', marginBottom: 5, display: 'block' };

  const handleSubmit = () => {
    if (!form.name.trim()) { alert('Student name is required'); return; }
    if (!form.room.trim()) { alert('Room number is required'); return; }
    addStudent(form);
    onClose();
  };
      ...
      <div style={{ display: 'grid', gap: 14 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            ...
            <input style={inputStyle} value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Arjun Rane" />
          </div>
          <div>
            <label style={labelStyle}>Room number *</label>
            <input style={inputStyle} value={form.room} onChange={e => set('room', e.target.value)} placeholder="e.g. Room 12" />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            ...
            <input style={inputStyle} value={form.email} onChange={e => set('email', e.target.value)} placeholder="student@email.com" />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div>
            <label style={labelStyle}>
              End date
              <span style={{ fontSize: 10, color: '#1D9E75', marginLeft: 6, fontWeight: 500 }}>auto (start + 30 days)</span>
            </label>
            <input type="date" style={readOnlyStyle} value={form.endDate} readOnly />
          </div>
        </div>

        {/* Date preview */}
        <div style={{ background: '#E1F5EE', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#0F6E56' }}>
          📅 Mess period: <strong>{formatDate(form.startDate)}</strong> → <strong>{formatDate(form.endDate)}</strong> (30 days)
        </div>

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 4 }}>
          <Button onClick={onClose}>Cancel</Button>
          <Button variant="primary" onClick={handleSubmit}>Add student</Button>
        </div>
      </div>
    </Modal>
  );
